...
class PiHLP:
    ...
    def setup_nodedata(self):
        ...
        for n in self.network.G_up.nodes:
            d = PiHLP_Node(self.network.totalnum_nodes)
        ...
        for n in self.network.G_up.nodes:
            for k in self.network.source_list:
                if self.nodedata[n].piflag[k] == 0:
                    self.nodedata[n].pival[k] = self.nodedata[n].pival[k]*(1-prb[k])
...
#         return IndxComputed
    ...
    def get_index_slot(self, src, thres_tau, ps_val):
#         print('ps_val',ps_val,'src',src, ps_val[src])
        IndxComputed = self.network.G_up.nodes[src]["Node"].index_finder_pre.compute_index(thres_tau, ps_val[src])
        return IndxComputed 
    
    def pre_emption_decision(self, currtime, pcvalue):
        for s in self.network.source_list:
            for d in self.network.commissioned_nodes[s]:
                last_rxd_pack = self.network.G_up.nodes[d]["Node"].get_latest_received_packet(s)
                if not last_rxd_pack == NOPACKET:
                    tdash = currtime - last_rxd_pack.generated_time
                    if (2/pcvalue[s]) < tdash:
                        packet = self.network.G_up.nodes[d]["Node"].remove_packet_from_hol(s)
    
    def get_activation_vector_slot(self, ps_val):
        index_computed = []
        for s in self.network.source_list:
            thres = self.nodedata[0].Abar[s] #Implementable IndexPOlicy #Using AoiiEstimate
#             thres = self.nodedata[0].aoii[s] #Ideal IndexPolicy
            indexobtained = self.get_index_slot(s, thres, ps_val)
